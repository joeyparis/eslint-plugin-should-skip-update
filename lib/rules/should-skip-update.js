  ...
  create: Components.detect((context, components) => {
    ...
    function reportUndeclaredSkipUpdateArguments(component, unusedVars) {
          ...
          variable.references.some((reference) => {
            // console.info(name, reference.identifier.parent.type)
              return [
                ...
                // 'VariableDeclarator', // I'm also not 100% sold on this one
                'UnaryExpression'
      ...
      const shouldIgnore = (name) => {
        ...
        // console.info(name, childrenNames)
        if (childrenNames.length === 0) {
          return false
        } else {
          // console.info(name, childrenNames, declaredDependencies)
          // console.info(name, !propIsUsed(name), childrenNames.every((name) => declaredDependencies.includes(name)))
      ...
      undeclareds = undeclareds.filter((propType) => {
        ...
        // console.info(buildFullName(propType.allNames), childrenNames)
        // console.info(childrenNames, undeclareds.map((n) => buildFullName(n.allNames)))
        // console.info(childrenNames, getImmediateChildrenNames(buildFullName(propType.allNames)))
        if (childrenNames.some((nn) => undeclareds.some((nnn) => buildFullName(nnn.allNames) === nn)))
          return false
        // console.info(buildFullName(propType.allNames), 'Still undeclared')
        return true
      })
