...
const { variablesInScope } = require('../util/variable')
  ...
  create: Components.detect((context, components) => {
    ...
    function reportUndeclaredSkipUpdateArguments(component, unusedVars) {
      ...
      const declaredNames = Array.from(new Set(component.usedPropTypes.flatMap((p) => {
        ...
        // References, parent type of JSXExpressionContainer
        // console.info(variablesInScope(context).filter((v) => v.name === 'bar')[0].references.map((r) => r.identifier))

        // console.info('component.usedPropTypes', component.usedPropTypes)

        // console.info(component.usedPropTypes)

      const propIsUsed = (name) => {
        return '?'
        // const lastName = name.split('.').pop()
        // console.info(variablesInScope(context).filter((v) => v.name === 'props')[0].references[0].from.block.params)
        // return variablesInScope(context).filter((v) => v.name === lastName).some((variable) => (
        //   variable.references.some((reference) => {
        //     // console.info(name, reference.identifier.parent.type)
        //       return [
      ...
      const shouldIgnore = (name) => {
        ...
        if (childrenNames.length === 0) {
          ...
          return childrenNames.every((name) => declaredNames.includes(name))
        }
      }
      ...
      const ignoredDeclaredNames = declaredNames.filter((n) => {
        ...
        // console.info(n, childrenNames, propIsUsed(n))
        // if (childrenNames.length === 0) {
          const usedNode = component.usedPropTypes.find((p) => p.allNames.join('.') === n)?.node//?.key

          // console.info(component.usedPropTypes, n)

          // console.info({usedNode})
          if (usedNode) {
            // console.info("blarghonk")
            // console.info(usedNode, unusedVars[1].node, /*equal(usedNode, unusedVars[0].node)*/)
            if (usedNode && unusedVars.some((v) => circularDeepEqual(v.node, usedNode))) {
              // console.info('blarg', unusedVars, declaredNames, n)
              // console.info(unusedVars[1].node)
              // console.info('bye')
              return true
            }
            // console.info('honkblarg')
            ...
            if (typeof usedNode.loc.identifierName === 'undefined') {
              ...
              // console.info({usedNodeName})
              // console.info(unusedVars)

              console.info(usedNodeName, unusedVars)
        ...
        // }
        // console.info('fish', n, childrenNames, propIsUsed(n), childrenNames.every((nn) => declaredDependencies.some((nnn) => {console.info(nn, nnn); return nn === nnn})))
        // return /*!propIsUsed(n) &&*/ childrenNames.every((nn) => declaredDependencies.some((nnn) => nn === nnn)) && !childrenNames.every((nn) => declaredNames.some((nnn) => nn === nnn.value));
        ...
        // if (!childrenNames.every((nn) => declaredDependencies.includes(nn)))
        //   true

        // if (childrenNames.length > 0) {

        //   const childrenNamesWithChildren = []
        //   const childrenNamesWithoutChildren = []

        //   childrenNames.forEach((name) => {
        //     if (getChildrenNames(name).length) {
        //       childrenNamesWithChildren.push(name)
        //     } else {
        //       childrenNamesWithoutChildren.push(name)
        //     }
        //   })

        //   if (childrenNamesWithoutChildren.every((nn) => declaredDependencies.includes(nn)))
        //     return true

        //   // if 
        //   console.info({
        //     childrenNamesWithChildren,
        //     childrenNamesWithoutChildren,
        //   })
        // }
      })

      // console.info({
      //   component: component.usedPropTypes[0].node.value,
      //   declaredNames,
      ...
      console.info('undeclareds', component.usedPropTypes, undeclareds.map((u) => u.allNames))
